...
export async function GET(
  ...
  const [_servicios] = await db
    .select({
      ...
      fecha_servicio: servicios.fechaServicio,
      feriado: servicios.feriado,
      importe_servicio: servicios.importeServicio,
      cliente: clientes.cliente,
      memo: servicios.memo,
      operarios: sql<
        Operario[]
      >`json_agg(json_build_object('nombre', ${operarios.nombre}, 'legajo', ${operarios.legajo}, 'hora_inicio', ${operariosServicios.horaInicio}, 'hora_fin', ${operariosServicios.horaFin}, 'a_cobrar', ${operariosServicios.aCobrar}, 'cancelado', ${operariosServicios.cancelado}))`.as(
        'operarios',
      ),
      recibos: sql<
        Recibo[]
      >`json_agg(json_build_object('recibo', ${recibos.recibo}, 'fecha_recibo', ${recibos.fechaRecibo}, 'importe', ${recibos.importeRecibo},'acopio',${recibos.acopio}))`.as(
        'recibos',
      ),
    })
    .from(servicios)
